...
class GbkParser:
    ...
    def get_mrna_exons(self):
        ...
        for alternative in self.t_dict['Alt transcripts']:
            ...
            try:
                ...
                alt_dict['NM_number'] = self.t_dict['genename']
                self.t_dict['refseqname'] = self.t_dict['genename']
                self.t_dict['genename'] = self.cds[0].qualifiers['gene'][0]
            exon = 1
    ...
    def get_protein(self):
        """
        This method takes the CDS tagged block from the GenBank features section and parses the contents to retrieve the
        protein sequence. This is added to the appropriate section of dictionary used to hold all required details of
        the file.
        """
        '''
        ...
        for alternative in self.t_dict['Alt transcripts']:
            ...
            self.t_dict['transcripts'][alternative]['protein_length'] = len(selected_cds.qualifiers['translation'][0])*3
            self.t_dict['transcripts'][alternative]['old_cds_offset'] = selected_cds.location.start
    ...
    def find_cds_delay(self):
        ...
        This involves taking the 'transcript start location' variable and calculating how many exonic bases that is from
        the first exonic base in the file. The transcript start location indicates the genomic position of the first
        coding base relative to the start of the reference file, whilst the required value is the number of exonic bases
        from first exon to first codon.
        For each exon, if the coding sequence start point coord. is after the exon end coord., add the length of the
        exon to the offset value. If the coding sequence starts between the start and end of the exon, add the length of
        the start point from the start of the exon
    ...
    def fill_and_find_features(self):
        ...
        """
        This section works on the assumption that each exon in the file will use the appropriate gene name and that the
        only relevant CDS and mRNA sections will also contain the same accession
        """
        try:
    ...
    def run(self):
        """
        This is the main method of the GBK Parser. This method is called after class instantiation and handles the
        operation of all the other functions to complete the dictionary which will hold all of the sequence and exon

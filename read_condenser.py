...
class Condenser:
    ...
    def create_file_pairings(self, gene, name_length, read):
        ...
        for filename in read_dict['transcripts']:
            file_pairs.append([read_dict['ref'],
                               filename,
                               self.create_file_name(gene, name_length,
                                                     filename, read)])
        return file_pairs
    ...
    def create_file_name(gene, name_length, filename, read):
        """
        Creates a new file name which combines the gene name, the transcript
        accession number and the read number
        """
        transcript = filename[name_length:name_length+1]
    ...
    def combine_files(read_pairs):
        """
        This will combine the created files in memory and write the output to
        a new file; the filename for the new file will be the one created in
        create_file_name()
        :param read_pairs:
        :return:
        """
